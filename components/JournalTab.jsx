...
export default function JournalTab() {
  ...
  async function doGenerate(regen=false) {
    try {
      ...
      // 2) تماس به /api/ai/image (از طریق generateSketch)
      ...
      // 3) ذخیره تصویر (IndexedDB) + متادیتا
  ...
  const recent = useMemo(()=> listRecent(10), [dateISO]); // صرفاً برای نمایش آرشیو نزدیک
    ...
    <div className="card" style={{ display:"grid", gap:12 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        ...
        <div className="sf-card" style={{ padding:12, display:"grid", gap:8 }}>
          <b>Sketch</b>
          <div style={{
            border:"1px dashed var(--border)", borderRadius:12, height:512, display:"grid",
            placeItems:"center", background:"var(--muted)"
          }}>
            {imgUrl ? (
              <img src={imgUrl} alt="journal sketch" style={{ width:"100%", height:"100%", objectFit:"contain", borderRadius:12 }}/>
            ) : (
      ...
      <div className="sf-card" style={{ padding:12 }}>
        ...
        <div style={{ display:"flex", gap:8, overflowX:"auto", marginTop:8 }}>
          ...
          {recent.map(e => (
            <button key={e.id} className="btn" style={{ padding:0, borderRadius:12, overflow:"hidden" }}
              ...
              <div style={{ width:120, height:120, background:"var(--muted)", display:"grid", placeItems:"center" }}>
                {e.thumb ? <img src={e.thumb} alt={e.id} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                         : <span className="hint">{e.id.slice(4,6)}/{e.id.slice(6)}</span>}
              </div>
              <div className="mono" style={{ padding:"4px 8px" }}>{e.id.slice(0,4)}-{e.id.slice(4,6)}-{e.id.slice(6)}</div>
            </button>
          ))}
        </div>
      </div>
