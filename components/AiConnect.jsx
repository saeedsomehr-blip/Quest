...
import { useAI } from "../ctx/AiContext.jsx"; // برای OAuth (اختیاری) 
...
export default function AiConnect() {
  ...
  async function testAndSave() {
    setStatus("Testing…");
    try {
      // تست با خود پروکسی (BYOK headers)
  ...
  function disableBYOK() {
    ...
    setStatus("BYOK disabled — proxy will use OAuth/.env");
  }
    ...
    <div className="card" style={{ display:"grid", gap:12 }}>
      ...
      <details open>
        ...
        <div style={{ display:"grid", gap:8, marginTop:8 }}>
          ...
          <div className="hint">
            اگر BYOK فعال باشد، همهٔ درخواست‌های ✨ به‌صورت پیش‌فرض با همین کلید/URL/مدل ارسال می‌شوند.
          </div>
        </div>
      </details>
    </div>
