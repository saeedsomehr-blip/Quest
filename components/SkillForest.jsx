...
const TREES = {
  ...
  [BR.STRENGTH]: {
    ...
    nodes: [
      ...
      { id: "str_brute",     label: "Brute",     x: 50, y: 50, req: { branch: BR.STRENGTH, at: 10 }, icon: "💪" },
  ...
  [BR.SOCIAL]: {
    ...
    nodes: [
      ...
      { id: "soc_orator",   label: "Orator",   x: 50, y: 50, req: { branch: BR.SOCIAL, at: 12 }, icon: "🎤" },
  ...
  [BR.TRADE]: {
    ...
    nodes: [
      ...
      { id: "trd_magnate", label: "Magnate", x: 85, y: 30, req: { branch: BR.TRADE, at: 20 }, icon: "🏛️" },
    ],
    edges: [
  ...
  [BR.HEALTH]: {
    ...
    nodes: [
      ...
      { id: "hlt_ironbody", label: "Iron Body", x: 75, y: 25, req: { branch: BR.HEALTH, at: 20 }, icon: "🛡️" },
    ],
    edges: [
...
function GraphNode({ node, xpMap, ownedPerks, accent }) {
  const cur = xpMap[node.req.branch] || 0;
  const unlocked = cur >= node.req.at;
  const owned = ownedPerks.includes(node.id) || false;

  const dim = 92;
  const style = {
    position: "absolute",
    left: `calc(${node.x}% - ${dim/2}px)`,
    ...
    textAlign: "center",
    fontSize: 12,
    transform: owned ? "scale(1.02)" : "none",
  ...
  return (
    <div className={`sf-node ${unlocked ? "unlocked" : "locked"} ${owned ? "owned" : ""}`}
         style={style}
         title={`${node.label} — Req ${node.req.branch} ${node.req.at}`}>
      <div style={{
        width: 26, height: 26, borderRadius: 999,
        background: unlocked ? accent : "var(--btn-hover)",
      ...
      <div style={{ fontWeight: 700, marginBottom: 2 }}>{node.label}</div>
      <div className="hint">Req: {node.req.at} {node.req.branch}</div>
      <div className="hint" style={{ marginTop: 2 }}>{owned ? "Owned" : unlocked ? "Unlocked" : "Locked"}</div>
    </div>
  );
}
...
function TraitTree({ traitKey, data, state, perks }) {
  const xp = state?.xp || {};
  const cur = xp[traitKey] || 0;

  const nextAt = useMemo(() => {
    const thresholds = data.nodes.map(n => n.req.at).sort((a,b)=>a-b);
    for (const t of thresholds) if (cur < t) return t;
    return null;
  }, [cur, data.nodes]);

  const pct = useMemo(() => {
    const max = Math.max(1, ...data.nodes.map(n => n.req.at));
        ...
        <div className="sf-head-right">
          <div className="sf-meter-wrap">
            ...
            <div className="sf-meter-caption">
              <span className="mono">{cur}</span> XP{" "}
