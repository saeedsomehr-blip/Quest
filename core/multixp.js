...
// - Reads PERK_RULES (skill + achievement + daily framework)
// - Supports tiered achievements and daily perks aggregation
// - Auto-migrates legacy key: qj_multixp_state_v1 → qj_multi_xp_v1
...
export function levelFromXP(xp) {
  let lvl = 0, sum = 0;
  while (true) { const need = xpNeededForLevel(lvl + 1); if (xp >= sum + need) { sum += need; lvl += 1; } else break; }
...
function blankState() {
  ...
  return {
    xp,
    perks: [],                 // permanent perk ids
    perkTiers: {},             // optional: { perkId: tier } for tiered achievements
    dailyPerks: { date: TODAY(), items: [] }, // computed daily effects
    synergies: [],
    skillsUnlocked: [],
    lastDecayOn: TODAY(),
  };
...
export function saveMultiXP(s) { try { localStorage.setItem(LS_KEY, JSON.stringify(s)); } catch {} }

// ───── Perk unlock (skills via branch thresholds)
...
// ───── Merge tiered Achievement perks into state (call from achievements flow)
export function mergePerksFromAchievements(ach) {
  ...
  for (const rule of PERK_RULES) {
    ...
    // if tiered, keep max tier observed
...
// ───── Apply awards with multipliers (permanent + daily + synergy)
function withPerkBoosts(state, awards) {
  const out = { ...awards };

  // 1) permanent perks (including tiered)
  let globalMul = 1.0;
  ...
  for (const id of active) {
    ...
    if (rule.tiered) {
      ...
      if (tier >= 1) {
        ...
        if (branch) {
          branchMul[branch] = (branchMul[branch] || 1.0) * m;
        } else {
          globalMul *= m;
        }
      }
  ...
  for (const t of Object.keys(out)) {
    ...
    const mul  = globalMul * (branchMul[t] || 1.0) * synergyExtra(t);
...
export function applyAwards(state, awardsMap) {
  const s = loadMultiXP();
  const withBoosts = withPerkBoosts(s, awardsMap);
  const next = { ...s, xp: { ...s.xp } };

  for (const [k, v] of Object.entries(withBoosts)) {
    if (!XP_TYPES.includes(k)) continue;
    next.xp[k] = Math.max(0, (next.xp[k] || 0) + Math.max(0, v | 0));
  }

  // skill-based unlocks + synergies
  ...
  saveMultiXP(next);
  return { state: next, perksNew, synNew, awarded: withBoosts };
}

// ────────────────────────────────────────────────────────────────────────────
// Public: compute a readable breakdown of active buffs (for UI previews)
export function computeBuffBreakdown(state) {
  const today = TODAY();
  const out = {
    globalMul: 1.0,
    branchMul: Object.fromEntries(XP_TYPES.map(t => [t, 1.0])),
    items: [],   // [{id,label,kind,scope:"global"|"branch", branch?:string, mult:number, source:"skill"|"ach"|"daily"|"synergy"}]
  };

  // 1) permanent perks (skill + achievement tiered)
  const active = new Set(state.perks || []);
  for (const id of active) {
    const rule = PERK_RULES.find(r => r.id === id);
    if (!rule) continue;
